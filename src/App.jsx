...
function fmtMins(m){ return m>=60?Math.floor(m/60)+"h"+(m%60?" "+m%60+"m":""):m+"min"; }
function fmtSecs(s){ const h=Math.floor(s/3600),m=Math.floor((s%3600)/60),ss=s%60; return h>0?`${h}:${String(m).padStart(2,"0")}:${String(ss).padStart(2,"0")}`:`${String(m).padStart(2,"0")}:${String(ss).padStart(2,"0")}`; }
function secsToMins(s){ return Math.round(s/60); }
function daysLabel(d,type){ if(d===null)return "—"; if(d<0)return type==="goal"?"Completed":"Done"; if(d===0)return "Today!"; return d+" day"+(d===1?"":"s")+" left"; }
...
function makeSeed(){
  ...
  ].forEach(({sid,tms})=>tms.forEach(({n,tps})=>{
    const tid=uid();themes.push({id:tid,subjectId:sid,name:n});
    tps.forEach(({n:tn,lv})=>topics.push({id:uid(),themeId:tid,subjectId:sid,name:tn,level:lv}));
  }));
...
function Btn({children,variant="ghost",size="md",disabled=false,style:sx={},...p}){
  const base={display:"inline-flex",alignItems:"center",gap:5,border:"none",borderRadius:8,fontFamily:"'Inter',sans-serif",fontWeight:600,cursor:disabled?"not-allowed":"pointer",whiteSpace:"nowrap",transition:"filter .12s",opacity:disabled?0.5:1};
  const sz={sm:{padding:"6px 13px",fontSize:11},md:{padding:"8px 16px",fontSize:12},xs:{padding:"4px 10px",fontSize:10}};
  const vr={accent:{background:C.accent,color:"#fff"},ghost:{background:"rgba(255,255,255,0.07)",color:C.text,border:`1px solid ${C.border}`},danger:{background:"rgba(248,81,73,0.12)",color:C.danger,border:`1px solid rgba(248,81,73,0.25)`},gold:{background:C.gold,color:"#000"},green:{background:C.ok,color:"#000"}};
...
function UBadge({level,active,onClick}){const u=UL[level];return(<span onClick={onClick} style={{display:"inline-flex",alignItems:"center",padding:"5px 12px",borderRadius:20,fontSize:11,fontWeight:700,cursor:"pointer",userSelect:"none",border:`1px solid ${u.bd}`,background:u.bg,color:u.color,opacity:active?1:0.25,transition:"opacity .15s"}}>{u.label}</span>);}
function SecLabel({children}){return(<div style={{display:"flex",alignItems:"center",gap:8,margin:"0 0 16px",fontSize:10,fontWeight:700,color:C.sub,textTransform:"uppercase",letterSpacing:"0.08em"}}>{children}<div style={{flex:1,height:1,background:C.border}}/></div>);}
function Modal({open,onClose,title,children,width=460}){if(!open)return null;return(<div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",backdropFilter:"blur(4px)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}><div onClick={e=>e.stopPropagation()} style={{background:C.surf,border:`1px solid ${C.border2}`,borderRadius:14,padding:28,width,maxWidth:"96vw",maxHeight:"92vh",overflowY:"auto",boxShadow:"0 24px 64px rgba(0,0,0,0.7)"}}><div style={{fontSize:16,fontWeight:700,marginBottom:22}}>{title}</div>{children}</div></div>);}
...
function FInput({style:sx={},...p}){return <input style={{width:"100%",padding:"10px 13px",background:C.surf2,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,fontFamily:"'Inter',sans-serif",fontSize:13,outline:"none",...sx}} {...p}/>;}
function FSelect({children,style:sx={},...p}){return <select style={{width:"100%",padding:"10px 13px",background:C.surf2,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,fontFamily:"'Inter',sans-serif",fontSize:13,outline:"none",appearance:"none",...sx}} {...p}>{children}</select>;}
function useToast(){
  const [msg,setMsg]=useState(""); const [vis,setVis]=useState(false); const t=useRef(null);
  const toast=useCallback(m=>{setMsg(m);setVis(true);clearTimeout(t.current);t.current=setTimeout(()=>setVis(false),2400);},[]);
...
export default function App(){
  ...
  const [openTh,setOpenTh]=useState({});
  ...
  const delTheme=id=>{setConfirmCfg({message:"Delete this theme and all its topics?",onOk:()=>{upd(n=>{n.themes=n.themes.filter(t=>t.id!==id);n.topics=n.topics.filter(t=>t.themeId!==id);});toast("Theme deleted");}});};
  const openPM=(tid=null)=>{if(!S.themes.length){toast("Add a theme first");return;}setTpTheme(tid||S.themes[0]?.id);setTpName("");setTpLv(1);setModal("topic");};
  const addTopic=()=>{if(!tpName.trim()||!tpTheme){toast("Please fill in all fields");return;}const th=S.themes.find(t=>t.id===tpTheme);upd(n=>{n.topics.push({id:uid(),themeId:tpTheme,subjectId:th.subjectId,name:tpName.trim(),level:tpLv});});setModal(null);toast("Topic added");};
  ...
  const addManualLog=(subjId,mins,note,date)=>{
    if(mins<1||!subjId)return;
    upd(n=>{if(!n.studyLogs)n.studyLogs=[];n.studyLogs.push({id:uid(),subjectId:subjId,date:date||dsOf(),mins,note:note||"",ts:Date.now()});});
  ...
  const buildPrompt=()=>{
    ...
    const ne=nextExam();
    const urgentNote=ne&&daysUntil(ne.date)<=14?"\nURGENT: "+ne.name+" is in "+daysUntil(ne.date)+" days. Prioritize heavily.":"";
    const lines=S.subjects.map(s=>{
      ...
      return s.name+": exam in "+(ex?daysUntil(ex.date)+"d":"N/A")+", mastery "+(p?p.pct+"%":"untracked")+", not-studied["+(ns.join(",")||"none")+"], uncertain["+(un.join(",")||"none")+"], remaining["+(rem.join(",")||"none")+"]";
    }).join("\n");
  ...
  const applyPastedPlan=text=>{
    ...
    try{
      ...
      const nameToId={};S.subjects.forEach(s=>{nameToId[s.name]=s.id;});
    ...
    }catch(e){setPlanErr("Error: "+e.message);}
  };
    ...
    return(<div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12,marginBottom:16}}>
        {[
          ...
          {lbl:"Next Exam",val:ne?daysUntil(ne.date)+"d":"—",note:ne?ne.name:"No exam set",color:ne&&daysUntil(ne.date)<=30?C.danger:C.text},
      ...
      <div style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:10,padding:"20px 22px",marginBottom:16}}>
        ...
        {S.exams.length?(<div style={{display:"flex",flexDirection:"column",gap:12}}>{[...S.exams].sort((a,b)=>a.date.localeCompare(b.date)).slice(0,4).map(e=>{
          ...
          const ac=d<0?C.ok:isGoal?(d<=30?C.warn:d<=90?C.accent:C.sub):(d<=14?C.danger:d<=30?C.warn:C.ok);
  ...
  const VExams=()=>{
    ...
    return(<div>
      <div style={{display:"flex",gap:6,marginBottom:18}}>
        {[["all","All",sorted.length],["exam","🎯 Exams",examCnt],["goal","📌 Goals",goalCnt]].map(([key,lbl,cnt])=>(<div key={key} onClick={()=>setTab(key)} style={{padding:"8px 16px",borderRadius:8,fontSize:13,fontWeight:600,cursor:"pointer",background:tab===key?"rgba(56,139,253,0.14)":"transparent",color:tab===key?C.accent:C.sub,transition:"all .12s"}}>{lbl} <span style={{fontFamily:"'DM Mono',monospace",fontSize:11,opacity:.7}}>({cnt})</span></div>))}
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:14,marginBottom:16}}>
        {filtered.map(e=>{
          ...
          return(<div key={e.id} style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:10,padding:"20px 22px",borderLeft:"4px solid "+ac}}>
            ...
            {p&&(<div style={{marginTop:14}}><div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:7}}><span style={{fontWeight:600,color:s.color}}>{s.icon} Mastery</span><span style={{fontFamily:"'DM Mono',monospace",color:C.sub}}>{p.pct}%</span></div><PBar pct={p.pct} color={s.color}/></div>)}
          </div>);
        })}
      </div>
              ...
              {S.planTasks.filter(t=>!t.completed).map(t=>{
                ...
                return (
                  <div key={t.id} onClick={()=>{
                    ...
                    if(!isSel){setTimerSubj(t.subject||S.subjects[0]?.id);setTimerNote(t.title);}
                    else{setTimerNote("");}
                  }}
  ...
  const views={dashboard:<VDashboard/>,plan:<VPlan/>,checklist:<VChecklist/>,schedule:<VSchedule/>,timer:<VTimer/>,subjects:<VSubjects/>,exams:<VExams/>,memo:<VNotes/>};
  const ne=nextExam();
  ...
  return(<div style={{display:"flex",height:"100vh",background:C.bg,color:C.text,fontFamily:"'Inter',sans-serif",fontSize:14,lineHeight:1.65}}>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
      <div style={{height:56,flexShrink:0,borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 28px",background:C.surf}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{background:C.surf2,border:`1px solid ${C.border}`,borderRadius:20,padding:"5px 14px",fontFamily:"'DM Mono',monospace",fontSize:11,color:C.gold}}>🎯 {ne?ne.name+" · "+daysUntil(ne.date)+"d":"No exam set"}</div>
    ...
    <Modal open={modal==="topic"} onClose={()=>setModal(null)} title="Add Topic">
      ...
      <FGrp label="Initial mastery level"><FSelect value={tpLv} onChange={e=>setTpLv(parseInt(e.target.value))}><option value={1}>🔴 Not studied</option><option value={2}>🟡 Uncertain</option><option value={3}>🟢 Mastered</option></FSelect></FGrp>
    ...
    <Modal open={modal==="exam"} onClose={()=>setModal(null)} title="Add Exam or Goal">
      <FGrp label="Type">
        <div style={{display:"flex",gap:8}}>
          {[["exam","🎯 Exam"],["goal","📌 Learning Goal"]].map(([key,lbl])=>(<div key={key} onClick={()=>setExType(key)} style={{flex:1,padding:"10px 0",borderRadius:8,textAlign:"center",fontSize:13,fontWeight:700,cursor:"pointer",border:`1px solid ${exType===key?C.accent:C.border}`,background:exType===key?"rgba(56,139,253,0.12)":"transparent",color:exType===key?C.accent:C.sub,transition:"all .15s"}}>{lbl}</div>))}
        </div>
      </FGrp>
